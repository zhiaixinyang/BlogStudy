...
public class OclockView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        RectF rectF = new RectF(distance, distance, distance + radius * 2, distance + radius * 2);
        canvas.drawArc(rectF, -85, 80, false, paint);

        //2、把时钟外层轮廓 补全,注意留下一点间距(给圆心角流出5度的宽余),也就是画圆弧,
        RectF rectF1 = new RectF(distance, distance, distance + radius * 2, distance + radius * 2);
        RectF rectF2 = new RectF(distance, distance, distance + radius * 2, distance + radius * 2);
        RectF rectF3 = new RectF(distance, distance, distance + radius * 2, distance + radius * 2);
        canvas.drawArc(rectF1, 5, 80, false, paint);
        canvas.drawArc(rectF2, 95, 80, false, paint);
        canvas.drawArc(rectF3, 185, 80, false, paint);
